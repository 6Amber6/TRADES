...
# =========================================================
# Gated fusion model (match 3-stage training)
# =========================================================
...
# Argument parser
# =========================================================
parser = argparse.ArgumentParser(description='PGD White-box Attack (3-Stage Gated Fusion)')
...
parser.add_argument('--model-path', required=True,
                    help='Path to fusion checkpoint (state_dict)')
...
def pgd_whitebox(model, X, y, epsilon, num_steps, step_size, random_start=True):
    ...
    err_pgd = (model(X_pgd).argmax(1) != y).float().sum()
...
def eval_adv_test_whitebox(model, loader):
    ...
    n = total
    print('='*60)
    print(f'natural_err_total: {int(natural_err_total)} / {n}')
    print(f'robust_err_total:  {int(robust_err_total)} / {n}')
    print(f'Natural accuracy:  {(1 - natural_err_total/n)*100:.2f}%')
    print(f'Robust accuracy:   {(1 - robust_err_total/n)*100:.2f}%')
    print('='*60)
...
def main():
    ...
    # Load single fusion checkpoint (3-stage training saves full state_dict)
    ...
    print(f'Architecture: WRN-{args.sub_depth}-{args.sub_widen}')
    print(f'PGD-{args.num_steps} attack, eps={args.epsilon}, step={args.step_size}')
    print()
