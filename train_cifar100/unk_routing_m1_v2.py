...
# Arguments
# =========================================================
parser = argparse.ArgumentParser(description='CIFAR-100 4x25+unk routing M1 (score=1-unk) TRADES')
...
parser.add_argument('--model-dir', default='./model-cifar100-4x25-unk-m1')
...
def main():
    ...
    if not resume_loaded:
        ...
        for i, loader in enumerate(group_loaders):
            ...
            milestones_sub = [int(args.epochs_sub * 0.5), int(args.epochs_sub * 0.75)]
            scheduler_sub = MultiStepLR(opt, milestones=milestones_sub, gamma=0.1)
            for ep in range(1, args.epochs_sub + 1):
                _, a = train_ce_epoch(m, loader, opt, device)
                scheduler_sub.step()
                print(f'[Sub][{ep}] {group_names[i]} acc={a*100:.2f}%, lr={opt.param_groups[0]["lr"]:.5f}')
    ...
    for ep in range(start_epoch, args.epochs_fusion + 1):
        ...
        if (ep % 10 == 0 and ep >= 60) or ep == args.epochs_fusion:
