...
parser.add_argument('--model-dir', default='./model-cifar100-4x25-unk-uniform')
...
def eval_routing_quality(fusion_model, test_group_loaders, test_loader, device, group_names):
    """Evaluate routing quality: global_w (full test set) + domain_w (each expert on its in-domain samples)."""
    fusion_model.eval()
    ...
    with torch.no_grad():
        for x, _ in test_loader:
            ...
            for m in fusion_model.submodels:
                ...
                unk_probs.append(p[:, -1:])
            unk_probs = torch.cat(unk_probs, dim=1)
            w = unknown_routing_scores(unk_probs, a=fusion_model.a, b=fusion_model.b, T=fusion_model.T)
            all_w.append(w.cpu())
