...
class SharedWRN20Gate(nn.Module):
    def __init__(self, depth=34, widen_factor=10, num_heads=20, num_known=5, unknown_idx=5,
                 gate_hidden=128, dropRate=0.0, fine_groups=None):
        super().__init__()
        assert ((depth - 4) % 6 == 0)
        n = (depth - 4) / 6
        ...
        self.block3 = nn.ModuleList([
            NetworkBlock(n, nChannels[2], nChannels[3], BasicBlock, 2, dropRate)
            for _ in range(num_heads)
        ])
        self.bn = nn.ModuleList([nn.BatchNorm2d(nChannels[3]) for _ in range(num_heads)])
        self.fc = nn.ModuleList([nn.Linear(nChannels[3], num_known + 1) for _ in range(num_heads)])
...
# Loss helpers
# =========================================================
def aux_ce_loss(all_logits, y, map_t_list, weight=0.02):
    loss = 0.0
    for i, logits in enumerate(all_logits):
        map_t = map_t_list[i]
        y_local = map_t[y]
        loss = loss + F.cross_entropy(logits, y_local)
    return weight * loss
...
def main():
    ...
    # Precompute mapping for aux loss (known classes + unknown)
    map_t_list = []
    ...
    train_loader_full = torch.utils.data.DataLoader(
        base_train, batch_size=args.batch_size, shuffle=True, num_workers=2, pin_memory=True
    )
    ...
    fusion = SharedWRN20Gate(depth=34, widen_factor=10, num_heads=20,
                             num_known=5, unknown_idx=5, fine_groups=coarse_to_fine).to(device)
    ...
    for ep in range(1, args.epochs_fusion + 1):
        ...
        for x, y in train_loader_full:
            ...
            loss += aux_ce_loss(aux_adv, y, map_t_list, weight=0.02)
        ...
        torch.save(fusion.state_dict(), f'{args.model_dir}/fusion-epoch{ep}.pt')
        with torch.no_grad():
            ema.apply_to(fusion)
            torch.save(fusion.state_dict(), f'{args.model_dir}/fusion-ema-epoch{ep}.pt')
            ema.restore(fusion)
