"""
AutoAttack evaluation for SoftRoutingFusion confidence variant
(train_trades_cifar10_softrouting_confidence.py).
This version uses confidence-only routing (no a/b weights).
...
Routing hyperparameters (--route-T, --route-margin) must match training values.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='AutoAttack eval – SoftRoutingFusion (confidence variant)')
    ...
    if args.log_path is None:
        ...
        args.log_path = os.path.join(model_dir, 'log-aa-softrouting-conf.log')
    ...
    print(f'Routing: T={args.route_T}, margin={args.route_margin}')
