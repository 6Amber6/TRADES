"""
AutoAttack evaluation for ParallelFusionWRN (train_trades_cifar10_parallel_warmup_ema_aux_lrstage.py).
...
Checkpoint formats accepted:
  - fusion-epoch{N}.pt  : raw state_dict  (saved by training script every 5 epochs)
...
# Allow running from repo root or auto_attack/ directory
...
class NormalizedModel(nn.Module):
    """Wraps a model so AutoAttack can work in [0,1] space."""
...
def main():
    parser = argparse.ArgumentParser(
        description='AutoAttack eval – ParallelFusionWRN (lrstage)')
    parser.add_argument('--model-path', required=True,
                        help='path to fusion checkpoint (fusion-epoch*.pt or checkpoint-last.pt)')
    ...
    if args.log_path is None:
        ...
        args.log_path = os.path.join(model_dir, 'log-aa-parallel.log')
