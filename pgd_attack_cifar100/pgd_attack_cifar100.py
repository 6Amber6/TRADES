...
parser = argparse.ArgumentParser(description='PyTorch CIFAR-100 PGD Attack Evaluation')
...
parser.add_argument('--model-path',
                    default='./model-cifar100-wideResNet/model-wideres-epoch200.pt',
...
def main():
    if args.white_box_attack:
        ...
        model_target = WideResNet(depth=34, widen_factor=10, num_classes=100).to(device)
        model_target.load_state_dict(torch.load(args.target_model_path))
        model_source = WideResNet(depth=34, widen_factor=10, num_classes=100).to(device)
